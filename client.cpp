...
public:
  ...
  void sendJoinRequest() {
    ...
    ENetPacket* packet = enet_packet_create("JOIN", 5, ENET_PACKET_FLAG_RELIABLE);
  ...
  void connect_client() {
    ...
    // Create ENet client
    std::cout << "Creating ENet client..." << std::endl;
    client = enet_host_create(NULL, 1, 2, 0, 0);  // NULL means no binding
    if (!client) {
            ...
            case ENET_EVENT_TYPE_RECEIVE: {
                if (event.packet->dataLength == sizeof(uint8_t)) {
                    ...
                    if (event.packet->dataLength == sizeof(LobbyUpdatePacket)) {
                      LobbyUpdatePacket* lobbyUpdate = (LobbyUpdatePacket*)event.packet->data;
                      
                      std::cout << "Received Lobby Update Packet - Players: " << (int)lobbyUpdate->numPlayers << std::endl;
